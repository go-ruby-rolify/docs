...
`go-ruby-rolify/rolify` reimplements Ruby's `rolify` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-rolify/rolify` directly, with no Ruby runtime;
...
Because `rolify` is pure Go with cgo disabled, it also compiles to **WebAssembly**
